...
// Retrieve the counter value from Firebase on page load
get(ref(db, "counter"))
  .then((snapshot) => {
    if (snapshot.exists()) {
      i = snapshot.val(); // Set the counter to the value stored in Firebase
    }
  })
  .catch((error) => {
...
// Event listener for the "Ajouter une commande" button
ajout.addEventListener('click', () => {
  createNewWrapper();
  first_ajout.remove(); // Remove the initial button after the first click
});
...
function createNewWrapper() {
  ...
  bodyScroll.appendChild(newWrapper); // Append the new wrapper to #body-scroll
